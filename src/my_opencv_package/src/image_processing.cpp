...
public:
    ImageProcessor() : Node("image_processor")
    {
        ...
        image_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/image_rect", 10, std::bind(&ImageProcessor::image_callback, this, std::placeholders::_1));
...
private:
    void image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
    	...
    	//RCLCPP_INFO(this->get_logger(), "Publishing contours");
        // Publish contours (Working perfectly!)
        ...
        // Publish thresholded binary image
        msg_ = cv_bridge::CvImage(std_msgs::msg::Header(), "mono8", thresholded_image).toImageMsg();
        thresholded_img_pub_->publish(*msg_.get());
        
        // FIX 1: Change "rgb8" to "bgr8" so colors don't invert
        msg_ = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", corner_image).toImageMsg();
        corner_img_pub_->publish(*msg_.get());
        
        // FIX 2: Safeguard the empty perspective image by publishing the gray image instead 
        // to prevent your node from encountering a Segmentation Fault.
        msg_ = cv_bridge::CvImage(std_msgs::msg::Header(), "mono8", gray_image).toImageMsg();
        prespective_img_pub_->publish(*msg_.get());
        
        // Publish ORB keypoints
